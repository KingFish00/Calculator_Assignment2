...
class ExpressionParser {
    ...
    fun evaluate(expression: String, precision: Int = 10): String {
        val usableExpression = convertToUsableExpression(expression)
        val res = evaluateExpression(usableExpression)
        if (res == "error")
        {
            return res
        }

        val result = res.toDouble()

        return roundToPrecision(result, precision).toString()
    }

    private fun convertToUsableExpression(expression: String): String {
    ...
    private fun evaluateExpression(expression: String): String {
        ...
        while (i < expression.length) {
            ...
            } else if (currChar.toString() isIn NormalOperators.values() ) {
                if (currChar == expression[i-1] || expression[i-1].toString() isIn NormalOperators.values()){
                    return "error"
                }
                /*else if (currChar == expression[i-1] && currChar.toString() == NormalOperators.MINUS.sign ) {
